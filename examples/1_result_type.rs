...
fn result_operations() {
    ...
    // println!("Unwrap err {}", err_int_string().unwrap());
    println!("Expect ok {}", ok_int_string().expect("Expected ok"));
    // println!("Expect err {}", err_int_string().expect("Expected ok"));
    ...
    println!("Result err {:?}", err_int_string().err());
    ...
    println!(
        "Result or chain {:?}",
        err_int_string().or(ok_int_string()).or(err_int_string())
    );
}
...
impl From<String> for MyComplexError {
    fn from(e: String) -> Self {
        MyComplexError(format!("My complex error from string : {e}"))
    }
}
